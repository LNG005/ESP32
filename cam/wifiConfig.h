...
//***************************************************************
#define RESET_PIN 2
#define RESET_HOLD_TIME 8000 // 8 seconds
#define DEBOUNCE_TIME 50     // 50ms cho chống nhiễu
...
const char html[] PROGMEM = R"html( 
  <!DOCTYPE html>
    ...
    <body>
      <div>
        <h3 style="text-align: center;">Thiết lập thông tin WiFi nhà bạn!!!</h3>
        <p id="info" style="text-align: center;">Đang tìm kiếm ...!</p>
        ...
        <button onclick="saveWifi()" style="background-color: #ADD8E6;margin-right: 10px">LƯU </button>
        <button onclick="reStart()" style="background-color: #FFA500;">RESTART ESP</button>
      </div>
...
void setupWifi()
{
  ...
  {
    Serial.println("Mạng WiFi ESP32 đã khởi tạo!");
    WiFi.mode(WIFI_AP);
    uint8_t macAddr[6];
    ...
    Serial.println("Địa chỉ IP:" + WiFi.softAPIP().toString());
    wifiMode = 0;
  }
}
...
public:
  void begin()
  {
    ...
    attachInterrupt(digitalPinToInterrupt(RESET_PIN), resetButtonISR, CHANGE);
  ...
  void run()
  {
    checkButton(); // Kiểm tra nút nhấn trước
    ...
    {
      // Nếu mất kết nối, thử kết nối lại mỗi 5 giây
